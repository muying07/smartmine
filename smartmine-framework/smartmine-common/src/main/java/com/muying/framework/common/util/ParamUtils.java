...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checksmartmineId(String smartmineId) {
        // 检查长度
        if (smartmineId.length() < ID_MIN_LENGTH || smartmineId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(smartmineId).matches();
    }
